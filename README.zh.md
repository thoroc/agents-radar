...
**`PAGES_URL`** — 在仓库变量中设置此项以配置你自己的 Web UI 基础 URL。
...
**`PAGES_URL`/feed.xml** — 在任意 RSS 阅读器（Feedly、Reeder、NewsBlur 等）中订阅，每日自动推送新简报。Feed URL 由你的 `PAGES_URL` 设置决定。Feed 包含最新 30 条报告（覆盖所有报告类型），与 `manifest.json` 同步更新。
