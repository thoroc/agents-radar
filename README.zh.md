...
每天早上 08:00 CST 自动运行的 GitHub Actions 工作流。聚合 10 个 AI 生态数据源，以多语言每日简报的形式发布为 GitHub Issues 并提交为 Markdown 文件。每周和每月自动生成汇总报告。
...
系统支持任何拥有 `locales/*.json` 文件的语言。目前已内置 21 个语言文件。添加新语言只需两步：

1. **添加 JSON 文件** `locales/{code}.json`，参照现有文件结构（由 `src/locale-schema.ts` 验证）
2. **在 `config.yml` 的 `languages` 列表中加入 ISO 代码**

无需修改代码。`src/i18n.ts` 的 `t(lang)` 函数在启动时加载所有语言文件，对缺失键自动回退为英文，所有支持语言的处理流程完全一致。

**语言选择优先级：**

1. `REPORT_LANGS` 环境变量（优先级最高，逗号分隔，如 `"en,zh,ja"`）
2. `config.yml` → `languages` 字段
3. 默认值：`["en", "zh"]`

所有报告生成、亮点提取、通知消息和 GitHub Issue 创建均会遍历该语言列表。启用的每种语言都会按比例增加 LLM 调用次数。

> 注意：系统的 LLM 提示指令和元数据标签均使用区域设置键。系统能够支持任何语言，但 LLM 对非英文和非中文的输出质量可能有所不同。欢迎社区提交翻译改进和提示词优化。
