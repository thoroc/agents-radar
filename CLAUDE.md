...
| `src/i18n.ts` | Locale system: lazy-loaded JSON reader, `t(lang)` accessor, `Lang` type, `validateLocale`, `SUPPORTED_LOCALES` |
...
| `src/locale-schema.ts` | Zod schema for locale file validation; derives `LocaleData` type | |
| `src/config.ts` | YAML config loader; `DEFAULT_LANGUAGES`, `getEnabledLangs()`, `RadarConfig` interface |
...
- All locale strings are stored in `locales/*.json` files (21 languages). Access them via `t(lang)` from `src/i18n.ts`. To add a new language, drop a valid JSON file into `locales/` and add its ISO code to the `languages` list in `config.yml` — no code changes needed.
