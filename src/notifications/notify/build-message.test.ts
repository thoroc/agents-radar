...
describe("buildMessage", () => {
  ...
  it("builds a daily message with zh + en reports", () => {
    const msg = buildMessage("2026-03-09", ["ai-cli", "ai-agents"], BASE_URL, undefined, ["zh", "en"]);
  ...
  it("shows weekly icon and suffix for weekly reports", () => {
    const msg = buildMessage("2026-03-09", ["ai-weekly"], BASE_URL, undefined, ["zh"], "zh");
    expect(msg).toContain("📅");
    expect(msg).toContain("周报");
  });

  it("shows monthly icon and suffix for monthly reports", () => {
    const msg = buildMessage("2026-03-09", ["ai-monthly"], BASE_URL, undefined, ["zh"], "zh");
    expect(msg).toContain("📆");
    expect(msg).toContain("月报");
  });

  it("monthly takes priority over weekly", () => {
    const msg = buildMessage("2026-03-09", ["ai-weekly", "ai-monthly"], BASE_URL, undefined, ["zh"], "zh");
    expect(msg).toContain("📆");
    expect(msg).toContain("月报");
  });

  it("renders zh-only reports without en link", () => {
    const msg = buildMessage("2026-03-09", ["ai-hn"], BASE_URL, undefined, ["zh"], "zh");
    expect(msg).toContain("HN 社区动态");
    expect(msg).not.toContain("HN Community");
  });

  it("includes Web UI and RSS links", () => {
    const msg = buildMessage("2026-03-09", ["ai-cli"], BASE_URL, undefined, ["zh"], "zh");
  ...
  it("includes highlights when provided", () => {
    ...
    const msg = buildMessage("2026-03-09", ["ai-cli", "ai-agents"], BASE_URL, highlights, ["zh", "en"]);
  ...
  it("works without highlights (null)", () => {
    const msg = buildMessage("2026-03-09", ["ai-cli"], BASE_URL, null, ["zh"], "zh");
    expect(msg).toContain("AI CLI 工具");
